        ...
        IConsumer<EntityDeletedEvent<NewsLetterSubscription>>
    {
        ...
        private void NotifyRegisteredWebHooks<T>(T entityDto, string webhookEvent, List<int> storeIds)
        {
            if (storeIds.Count > 0)
            {
                ...
                WebHookManager.NotifyAllAsync(new NotificationDictionary[] { new NotificationDictionary(webhookEvent, entityDto) }
            ...
            {
                WebHookManager.NotifyAllAsync(new NotificationDictionary[] { new NotificationDictionary(webhookEvent, entityDto) }, predicate: null);
            }
        }
        ...
        private void NotifyUnmappedEntityWebhooks<T>(T entityDto, List<int> storeIds)
        {
            if (typeof(T) == typeof(ProductDto))
            {
                ...
                WebHookManager.NotifyAllAsync(new NotificationDictionary[] { new NotificationDictionary(WebHookNames.ProductsUnmap, entityDto) },
                    (hook, hookUser) => !IsEntityMatchingTheWebHookStoreId(hookUser, storeIds));
            }
            else if (typeof(T) == typeof(CategoryDto))
            {
                ...
                // Notify all webhooks that the entity is not mapped to their store.
                WebHookManager.NotifyAllAsync(new NotificationDictionary[] { new NotificationDictionary(WebHookNames.CategoriesUnmap, entityDto) },
                    (hook, hookUser) => !IsEntityMatchingTheWebHookStoreId(hookUser, storeIds));
            }
        }
